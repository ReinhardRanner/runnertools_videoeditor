  ...
  srcDoc={`
    ...
    <script>
      ...
      window.addEventListener('unhandledrejection', function(e) {
        parent.postMessage({ type: 'iframe-error', message: 'Unhandled Promise: ' + (e.reason || e) }, '*');
      });
    ...
    </script>` + generatedHtml}
  className="w-full h-full border-none"
  style={{ width: '100%', height: '100%' }}
/>
                  </div>
                  {iframeErrors.length > 0 && (
                    <div className="max-h-24 overflow-auto rounded-lg bg-red-950/80 border border-red-500/20 p-3 flex gap-3">
                      <div className="flex-1 min-w-0 space-y-1">
                        {iframeErrors.map((err, i) => (
                          <p key={i} className="text-[10px] font-mono text-red-400/90 leading-relaxed">
                            <span className="text-red-500 font-bold mr-1.5">ERROR</span>{err}
                          </p>
                        ))}
                      </div>
                      <button
            ...
            <div className="p-8 bg-white/[0.02] border-t border-white/5 space-y-6">
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-3 py-4 rounded-2xl font-bold text-[11px] uppercase tracking-[0.2em] transition-all active:scale-[0.98] disabled:opacity-30 ${
                    isManim || !generatedHtml ? 'bg-white/5 hover:bg-white/10 text-white' : 'bg-white/5 hover:bg-white/10 text-white'
                  }`}
                >
                  ...
                  <SplitButton
                    options={[
                      { 
                        ...
                        buttonLabel: 'Render', // Different shorter text
                      ...
                      { 
                        ...
                        buttonLabel: 'Render', // Shorter text for the button
